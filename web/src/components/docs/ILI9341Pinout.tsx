...
export function ILI9341Pinout() {
  ...
  }).filter(Boolean) as Array<{ from: { x: number; y: number }; to: { x: number; y: number }; color: string; piPin: number; idx: number; group: string }>;

  const renderPin = (pin: typeof PI_PINS[number][number], side: "left" | "right") => {
    const isConn = highlightSet.has(pin.num);
    const isHov = hovered === pin.num;
    const anyHov = hovered !== null;
    const c = typeColors[pin.type];
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "4px", width: "175px", justifyContent: side === "left" ? "flex-end" : "flex-start" }}
        ...
        {side === "left" && (
          <span style={{
            ...
            fontWeight: isHov ? 700 : 400, transition: "all .2s", textAlign: "right" as const, flex: 1, whiteSpace: "nowrap" as const, overflow: "hidden",
          }}>
        ...
        {side === "right" && (
          <span style={{
            fontSize: "9px",
            color: isHov ? (colorMap[pin.num] || "#f0f0f0") : anyHov && !isHov ? "#333" : "#666",
            fontWeight: isHov ? 700 : 400, transition: "all .2s", whiteSpace: "nowrap" as const, overflow: "hidden",
          }}>
            {pin.label}
    ...
    <div style={{ color: "#e2e8f0", padding: "12px 0" }}>
      <div style={{ display: "flex", gap: "8px", justifyContent: "center", marginBottom: "20px" }}>
        <button onClick={() => { setShowLcd(!showLcd); setHovered(null); }}
          style={{
            ...
            fontSize: "11px", fontWeight: showLcd ? 700 : 400,
          }}>
        ...
        <button onClick={() => { setShowTouch(!showTouch); setHovered(null); }}
          style={{
            ...
            fontSize: "11px", fontWeight: showTouch ? 700 : 400,
          }}>
      ...
      <div ref={containerRef} style={{ position: "relative", display: "flex", justifyContent: "center", gap: "0px", alignItems: "flex-start", maxWidth: "920px", margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: "13px", fontWeight: 700, color: "#f97316", textAlign: "center", marginBottom: "2px" }}>
            ILI9341 LCD
          </div>
          <div style={{
            fontSize: "9px", color: "#475569", textAlign: "center", marginBottom: "12px",
            borderBottom: "1px solid #1e293b", paddingBottom: "10px",
          }}>
            3.5" 320x480 + XPT2046 Touch
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            {showLcd && (
              <>
                <div style={{ fontSize: "9px", color: "#f97316", textTransform: "uppercase" as const, letterSpacing: "0.08em", marginBottom: "2px", marginTop: "4px" }}>
                  Display
                </div>
            ...
            {showTouch && (
              <>
                <div style={{ fontSize: "9px", color: "#a3e635", textTransform: "uppercase" as const, letterSpacing: "0.08em", marginBottom: "2px", marginTop: "8px" }}>
                  Touch (XPT2046)
                </div>
