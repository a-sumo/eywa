...
The real-time spatial layer. Each fold is a subspace where agents log operations, share context, and track progress toward a destination.

**What exists today:**
- MCP server (Cloudflare Worker) with 50+ tools for session lifecycle, memory, collaboration, recovery, and network
- Web dashboard (React/Vite) with HubView, OperationsView, SeedMonitor, FoldsIndex, AgentDetail timeline, Knowledge Hub
...
- CLI (`npx eywa-ai`) with fold setup, status, seeds, claims, metrics, inbox, approve, tasks commands
- Spectacles AR panel (tile streaming proven, voice integration in progress)
- Pi display scripts (e-ink, TFT touch)

**Key patterns:**
- Supabase PostgREST for persistence (folds, memories, messages tables)
...
- Fold knowledge browsing (in progress)
- Cross-fold search (in progress)
...
- voicePrompt.ts: system prompt builder with fold context seeding
- Route wired at /f/:slug/voices, accessible from mobile tabs

**What's missing:**
- Spectacles integration (EywaGeminiLive.ts exists but needs scene wiring to tile panel)
- Voice commands for navigation (switch folds, compare agents, set course)
...
- Folds index for cross-subspace awareness
...
The landing page exists but there is no guided onboarding after fold creation. A new user creates a fold and sees an empty HubView with no agents, no destination, no context. They have no idea what to do next. Build:
...
- An interactive onboarding overlay or wizard in HubView that appears for empty folds
- Pre-populated demo content in demo folds so the "Try Demo" button leads to a fold that already shows what Eywa looks like with active agents

### Priority 2: Fold knowledge browsing UI [IN PROGRESS]
Fold-local knowledge (eywa_learn/eywa_knowledge entries) has no browsing UI. Agents store architecture decisions, conventions, and gotchas via eywa_learn, but humans can only see them through the MCP tools. Build a FoldKnowledge component at /f/:slug/knowledge that shows, searches, and allows deletion of knowledge entries. Claimed by fair-bear.
...
### Priority 4: Cross-fold search [IN PROGRESS]
Search across all folds for agents, knowledge entries, or operations. The folds index shows per-fold stats but can't answer "which fold has agent X?" or "where was decision Y made?" Claimed by bright-thorn.

### Priority 5: Fold-level Supabase Realtime [IN PROGRESS]
The FoldsIndex fetches all fold stats on mount and polls every 30s. Adding Realtime subscriptions would make it live. Same applies to SeedMonitor when viewing a fold you're not in. Claimed by swift-stone.

### Priority 6: User model and personal voice [NEW]
VISION.md identifies this as the biggest gap: no concept of "you" in the system. Every user sees the same fold view. The system speaks in system language, not personal language. Build:
...
The voice interface is partially built but not end-to-end functional. Wire EywaGeminiLive into the Lens Studio scene for Spectacles AR voice. Add voice navigation commands (switch folds, compare agents, set course). Add push-to-talk for noisy environments.
...
Folds index view at /folds with live stats per fold. Shipped in 976fb69.
